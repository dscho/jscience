 ...
 *     [/code]</p>
 * <p> As for any concrete {@link org.jscience.mathematics.structures.Structure
...
public final class DenseMatrix<F extends Field<F>> extends Matrix<F> {
    ...
    public DenseMatrix<F> times(Matrix<F> that) {
        ...
        if (that.getNumberOfRows() != _n)
            throw new DimensionException();
        DenseMatrix<F> M = DenseMatrix.newInstance(p, true); // Transposed.
    ...
    private void multiply(
            Matrix<F> that, Index startColumn, Index endColumn, FastTable<DenseVector<F>> columnsResult) {
        ...
        if (end - start < 32) {  // Direct calculation.
            ...
            for (int j = start; j < end; j++) {
                ...
                for (int i = 0; i < m; i++) {
                    DenseVector<F> thisRowi = this._rows.get(i);
                    column._elements.add(thisRowi.times(thatColj));
                }
            }   
    ...
    private static ConcurrentContext.Logic MULTIPLY = new ConcurrentContext.Logic() {
        ...
        public void run() {
            DenseMatrix _this = getArgument(0);
    ...
    DenseMatrix<F> copy() {
        ...
        for (int i = 0, m = _rows.size(); i < m; i++) {
            DenseVector<F> V = DenseVector.newInstance();
            V._elements.addAll(_rows.get(i)._elements);
            M._rows.add(V);
        }
        return M;
    }
    ...
    private static Factory<DenseMatrix> FACTORY = new Factory<DenseMatrix>() {
        @Override
