...
public final class SparseMatrix<F extends Field<F>> extends Matrix<F> {
    ...
    public SparseMatrix<F> times(Matrix<F> that) {
        ...
        if (that.getNumberOfRows() != n)
            throw new DimensionException();
        SparseMatrix<F> M = SparseMatrix.newInstance(p, _zero, true);
        for (int j = 0; j < p; j++) {
            ...
            for (int i = 0; i < m; i++) {
                Vector<F> thisRowi = this.getRow(i);
                F e = thisRowi.times(thatColj);
                if (!_zero.equals(e)) {
    ...
    private static Factory<SparseMatrix> FACTORY = new Factory<SparseMatrix>() {
        @Override
