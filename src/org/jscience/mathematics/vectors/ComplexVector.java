...
public final class ComplexVector extends Vector<Complex> implements
    ...
    public ComplexVector opposite() {
        ComplexVector V = ComplexVector.newInstance(_dimension);
    ...
    static ComplexVector newInstance(final int dimension) {
        final ComplexVector V = FACTORY.object();
        V._dimension = dimension;
        if ((V._reals == null) || (V._reals.length < dimension)) {
            MemoryArea.getMemoryArea(V).executeInArea(new Runnable() {
                public void run() {
                    V._reals = new double[dimension];
                    V._imags = new double[dimension];
                }
            });
        }
        return V;
    ...
    private static Factory<ComplexVector> FACTORY = new Factory<ComplexVector>() {
