...
public final class ProductUnit<Q extends Quantity> extends DerivedUnit<Q> {
    ...
    public Unit<? super Q> getBaseUnits() {
        ...
        Unit baseUnits = ONE;
        for (int i = 0; i < _elements.length; i++) {
            Unit unit = _elements[i]._unit.getBaseUnits();
    ...
    public UnitConverter toBaseUnits() {
        if (isProductOfBaseUnits()) return UnitConverter.IDENTITY; 
        double factor = 1.0;
        for (int i = 0; i < _elements.length; i++) {
            UnitConverter cvtr = _elements[i]._unit.toBaseUnits();
            if (cvtr.isLinear()) {
                factor *= Math.pow(cvtr.derivative(0),
