...
public abstract class UnitFormat extends Format {
     ...
     */
    public final StringBuffer format(Object unit, StringBuffer toAppendTo,
            FieldPosition pos) {
        try {
    ...
    private static class StandardFormat extends UnitFormat {
        ...
        public String nameFor(Unit unit) {
            ...
            if (unit instanceof TransformedUnit) {
                ...
                Unit baseUnits = tfmUnit.getBaseUnits();
                UnitConverter cvtr = tfmUnit.toBaseUnits();
    ...
    static {
        ...
        for (int i = 0; i < PREFIXES.length; i++) {
            ...
            STANDARD.label(SI.KILOGRAM.times(CONVERTERS[i].derivative(0) * 1e-3),
                        PREFIXES[i] + "g");
            if (PREFIXES[i] == "µ") {
                 ASCII.label(SI.KILOGRAM.times(CONVERTERS[i].derivative(0) * 1e-3), "microg");
            }   
        }
        ...
        STANDARD.label(SI.CELSIUS, "°C");
        ASCII.label(SI.CELSIUS, "Celsius");
        for (int i = 0; i < PREFIXES.length; i++) {
            STANDARD.label(SI.CELSIUS.transform(CONVERTERS[i]), PREFIXES[i] + "°C");
