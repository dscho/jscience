...
public class DiagonalMatrix<F extends Field<F>> extends SparseMatrix<F> {
     ...
     */
    public static <F extends Field<F>> DiagonalMatrix<F> valueOf(Vector<F> that) {
        DiagonalMatrix M = FACTORY.object();
    ...
    public F get(int i, int j) {
        if ((j < 0) || (j > _diagonal.getDimension()))
    ...
    public SparseVector<F> getRow(int i) {
        return SparseVector.valueOf(i, _diagonal.get(i), _diagonal.getDimension());
    }
    ...
    private DiagonalMatrix<F> times(DiagonalMatrix<F> that) {
        final int n = _diagonal.getDimension();
        if (that._diagonal.getDimension() != n)
