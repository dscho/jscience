 ...
 */
final class SparseMatrixImpl<F extends Field<F>> extends SparseMatrix<F> {

    /**
     * Holds the object factory.
     */
    static ObjectFactory<SparseMatrixImpl> FACTORY = new ObjectFactory<SparseMatrixImpl>() {

        @Override
        protected SparseMatrixImpl create() {
            return new SparseMatrixImpl();
        }

        @Override
        protected void cleanup(SparseMatrixImpl matrix) {
            matrix._rows.reset();
        }
    };
    ...
    public static <F extends Field<F>> SparseMatrixImpl<F> valueOf(Vector<F>... rows) {
        return SparseMatrixImpl.valueOf(Arrays.asList(rows));
    }

    // See parent static method.
    public static <F extends Field<F>> SparseMatrixImpl<F> valueOf(List<? extends Vector<F>> rows) {
